...
def book():
    ...
    if request.method == "POST":
        appointment = Appointment(
            ...
            doctor_assigned=request.form['doctor_name'] 
        )
    ...
    return render_template('book.html', doctors=doctors)  # Pass doctors to template
...
def approve_appointment(appointment_id):
    ...
    if appointment:
        ...
        if user:  # Ensure user exists before sending an email
            msg = Message("Appointment Approved", recipients=[user.email])
            msg.body = f"Dear {user.name}, your appointment for {appointment.problem} has been approved with Dr. {appointment.doctor_assigned}."
            mail.send(msg)
...
if __name__ == "__main__":
    with app.app_context():
        ...
        # Check if doctors exist before adding them
        if Doctor.query.count() == 0:
            doctor1 = Doctor(name="John Doe", specialization="Cardiologist")
            doctor2 = Doctor(name="Jane Smith", specialization="Neurologist")
            doctor3 = Doctor(name="Ramesh", specialization="Bone")
            doctor4 = Doctor(name="Suresh", specialization="Neur")
